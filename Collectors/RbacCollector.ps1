...
.DESCRIPTION
    ...
    Loops through every subscription the current identity logged in can access.
...
function Resolve-RbacScopeType {
    ...
    # this filters the data for role assignment at the managment group layer. 
    if ($Scope -match '^/providers/Microsoft\.Management') { return 'ManagementGroup' }

    $parts = $Scope.TrimStart('/').Split('/')

    # gets role assignment at /subscriptions/{id}
    if ($parts.Count -eq 2 -and $parts[0] -eq 'subscriptions') {
        return 'Subscription'
    }

    # this seeraches for role assingment at the resource group layer
    if ($parts.Count -eq 4 -and $parts[2] -eq 'resourcegroups') {
        return 'ResourceGroup'
    }

    # goes deeper for direct resource role assignment
    if ($parts.Count -gt 4) {
        return 'Resource'
    }
    ...
    try {
        ...
        foreach ($ra in $raw) {
            ...
            $results.Add([PSCustomObject]@{
                ...
                # If assigned directly to a user = Direct assignment 
                # If via group/servie principle  = Inherited assignment 
...
function Get-RbacSnapshot {
    ...
    # Gets all subscriptions in the tenant the current logged in user can access
    try {
